...
class WireGuardEndpointTests: XCTestCase {
    
    func test_init() {
        let endpoint1 = ((try? WireGuardEndpoint(endpointString: "10.0.0.0:53")) as WireGuardEndpoint??)
        XCTAssertEqual(endpoint1??.ipAddress, "10.0.0.0")
        XCTAssertEqual(endpoint1??.port, 53)
        XCTAssertEqual(endpoint1??.addressType, .IPv4)
        
        let endpoint2 = ((try? WireGuardEndpoint(endpointString: "[2001:db8:a0b:12f0::1]:21")) as WireGuardEndpoint??)
