...
extension ApiService {
    ...
    func getServersList(storeInCache: Bool, completion: @escaping (ServersUpdateResult) -> Void) {
        ...
        log(.info, message:  "Fetching servers list...")
            ...
            case .success(let response):
                ...
                if let data = response.body {
                    ...
                    if serversList.servers.count > 0 {
                        log(.info, message: "Fetching servers list completed successfully")
                        completion(.success(serversList))
                        return
                    }
                }
                
                log(.info, message: "Error updating servers list (probably parsing error)")
                completion(.error)
            case .failure:
                log(.info, message: "Error fetching servers list")
                completion(.error)
            }
        }
    }
